...
@dataclass
class SDFNSRFieldConfig(FieldConfig):
    """SDF NSR Field Config"""

    _target: Type = field(default_factory=lambda: SDFNSRField)
    num_layers: int = 8
...
class SDFNSRField(Field):
    ...
    def get_point_sdf(self, x, skip_spatial_distortion=False):
        """compute the gradient of the ray"""
        if self.spatial_distortion is not None and not skip_spatial_distortion:
            x = self.spatial_distortion(x)
        points_sdf = self.forward_geonetwork(x)[..., 0]
