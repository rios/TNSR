...
@dataclass
class SurfaceNSRModelConfig(ModelConfig):
    """Surface Model Config"""

    _target: Type = field(default_factory=lambda: SurfaceNSRModel)
    ...
    sdf_field: SDFNSRFieldConfig = field(default_factory=SDFNSRFieldConfig)
...
class SurfaceNSRModel(Model):
    ...
    def populate_modules(self):
        ...
        if self.config.background_model == "grid":
            self.field_background = NerfactoField(
                ...
                num_images=self.num_train_data,
                use_average_appearance_embedding=self.config.use_average_appearance_embedding,
                appearance_embedding_dim=0, # no camera embedding feature
            )
    ...
    def crop_center(self, image, ratio=0.8):
        # Assuming `image` is a tensor with shape [1, C, H, W]
        _, _, h, w = image.shape
        crop_w = int(w * ratio)
        start_w = (w - crop_w) // 2
        return image[:, :, :, start_w:start_w + crop_w]
    ...
    def get_image_metrics_and_images(
        ...
        if "bg_rgb" in outputs.keys():
            combined_rgb = torch.cat([image, rgb, outputs["bg_rgb"]], dim=1)
        else:
